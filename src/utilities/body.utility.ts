...
export class BodyUtility {
   ...
   * @param {boolean} resolveGrouping - A boolean flag indicating whether to group pull requests by
   * related issues detected from the resolve line. If set to false, pull requests will be listed flat
   * instead of being grouped under their related issues.
   ...
   * @param {boolean} resolveGrouping - A boolean flag indicating whether to extract related issues
   * from the resolve line. If false, the resolve line issue detection is skipped entirely.
   * @param {string[]} excludeKeywords - `excludeKeywords` is an array of strings that contains
  ...
  private extractPostReleaseChecklistItems(
    prsWithIssues: Map<PrEntry, string[]>,
    sectionTitle: string
  ): Map<PrEntry, ChecklistItem[]> {
    const result = new Map<PrEntry, ChecklistItem[]>()

    for (const pr of prsWithIssues.keys()) {
      const body = pr.body ?? ''
      if (!body) continue

      const rawItems: {indent: number; text: string}[] = []
   ...
   */
  private checklistItemKey(prIdentifier: string, text: string): string {
